...
for i in range(6000, 12000):
	values = (arr[i], password, "Crewmate")
	mycursor.execute(sql, values)
...
for i in range(12000, 12050):
	values = (arr[i], password, "Offshore_Management")
	mycursor.execute(sql, values)
'''
#------------------------------------------------------------------------------------------------------------------#
...
try:
	mycursor.execute("select ID from Login where Type = 'Passenger'")#Execute SQL Query to select all record   
...
for i in range(0, 2000):
	...
	if i < 500:
		...
		j = 4
	temp = str(random.randint(1111111111111, 9999999999999))
	while temp in arr:
		temp = str(random.randint(1111111111111, 9999999999999))
	values = (year+j*10000+cruise_indexer*10+1, names[name_indexer], random.randint(10, 75), temp, room_distribute[room_checker], random.randint(0,1),random.randint(0,1), logins[login_checker])
	#print(values)
...
for i in range(0, 2400):
	...
	name_indexer = name_indexer + 1
	room_checker = (room_checker + 1) % 600
	login_checker = (login_checker + 1)
	cruise_indexer = (cruise_indexer + 1) % 601
	mycursor.execute(sql, values)
